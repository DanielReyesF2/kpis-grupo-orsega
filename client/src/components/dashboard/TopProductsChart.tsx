/**
 * Top Products Chart - Gráfica moderna de productos más vendidos
...
export function TopProductsChart({ 
  ...
  const companyConfig = companyId === 2
    ? {
        ...
        gradient: ["#8b5cf6", "#7c3aed", "#6366f1", "#818cf8", "#a78bfa"]
      }
    : {
        primary: "#10b981",
        secondary: "#34d399",
        light: "rgba(16, 185, 129, 0.15)",
        gradient: ["#10b981", "#059669", "#0d9488", "#14b8a6", "#34d399"]
      };
      ...
      <Card className="overflow-hidden border-dashed border-2">
        <CardContent className="py-16">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground font-medium">No hay datos de productos disponibles</p>
            <p className="text-xs text-muted-foreground/60 mt-1">Sube datos de ventas para ver los top productos</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  const totalVolume = topProducts.reduce((sum, p) => sum + p.volume, 0);
  const totalClients = topProducts.reduce((sum, p) => sum + p.uniqueClients, 0);
  const totalTransactions = topProducts.reduce((sum, p) => sum + p.transactions, 0);
  const topProductPercent = totalVolume > 0 ? ((topProducts[0].volume / totalVolume) * 100).toFixed(1) : 0;
  const unit = topProducts[0]?.unit || (companyId === 2 ? "unidades" : "KG");
    ...
    return (
      <div className="bg-popover/95 backdrop-blur-sm border border-border rounded-xl p-4 shadow-xl min-w-[220px]">
        <div className="flex items-center gap-2 mb-3">
          {data.rank === 1 && <Star className="h-4 w-4 text-amber-500 fill-amber-500" />}
          <p className="text-foreground font-bold">{data.fullName}</p>
        </div>
        <div className="space-y-2">
          <div className="flex justify-between items-center gap-4">
            <span className="text-muted-foreground text-sm">Volumen vendido</span>
            <span className="text-foreground font-bold">{formatFullNumber(data.volume)} {data.unit}</span>
          </div>
          <div className="flex justify-between items-center gap-4">
            <span className="text-muted-foreground text-sm">Clientes únicos</span>
            <span className="text-foreground font-medium">{data.uniqueClients}</span>
          </div>
          <div className="flex justify-between items-center gap-4">
      ...
      <CardHeader className="pb-0">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            >
              <ShoppingBag className="h-6 w-6" style={{ color: companyConfig.primary }} />
            </div>
            <div>
          ...
          >
            ...
            <span>#{1} = {topProductPercent}% del total</span>
          </div>
        </div>
            ...
            >
              <defs>
                {chartData.map((_, index) => (
                  <linearGradient key={`gradient-product-${index}`} id={`productGradient-${index}`} x1="0" y1="0" x2="1" y2="0">
              ...
              >
                {chartData.map((entry, index) => (
                  <Cell
                    ...
                    fill={index === 0 ? '#f59e0b' : `url(#productGradient-${index})`}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{product.uniqueClients} clientes · {product.transactions} transacciones</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold" style={{ color: index === 0 ? '#f59e0b' : companyConfig.primary }}>
                    {formatFullNumber(product.volume)}
                  </p>
