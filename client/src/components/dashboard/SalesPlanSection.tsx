...
import { ClipboardList, Target, DollarSign, Users, AlertTriangle } from "lucide-react";
...
export function SalesPlanSection({ companyId, year, month }: SalesPlanSectionProps) {
  ...
  const warningClients = insights?.focusClients?.warning ?? [];
  ...
  const totalClientsToContact = criticalCount + warningClients.length;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/30">
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            {/* Opportunities Count */}
            <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200/50 dark:border-emerald-800/30">
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-200/50 dark:border-blue-800/30">
              ...
              <p className="text-xs text-muted-foreground mt-1">30% del revenue en riesgo</p>
            </div>
              ...
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${
                    churnRisk >= 70 ? 'bg-red-500' :
                    churnRisk >= 40 ? 'bg-amber-500' :
                    'bg-emerald-500'
                  }`}
                  style={{ width: `${Math.min(churnRisk, 100)}%` }}
                />
              </div>
              <p className="text-right text-xs text-muted-foreground mt-1">{churnRisk.toFixed(1)}%</p>
            </div>
          </div>
        </div>
