...
export function registerRoutes(app: express.Application) {
  ...
  app.post("/api/sales/upload", jwtAuthMiddleware, uploadLimiter, (req, res, next) => {
    ...
    try {
      ...
      if (!file) {
        console.error('❌ [Sales Upload] No se recibió ningún archivo');
        return res.status(400).json({
          error: 'No se subió ningún archivo',
      ...
      if (!resolvedCompanyId) {
        return res.status(400).json({
          ...
          details: 'Debes proporcionar un companyId o estar asociado a una empresa'
        });
      }

      // Verificar permisos
      if (user.role !== 'admin' && user.companyId !== resolvedCompanyId) {
        return res.status(403).json({
          error: 'No tienes permisos para subir archivos para esta empresa'
        });
      }

      console.log(`📊 [Sales Upload] Procesando archivo para empresa ${resolvedCompanyId}`);
      ...
      console.log(`📋 [Sales Upload] Hoja encontrada: "${worksheet.name}" con ${worksheet.rowCount} filas`);

      // Buscar la fila de encabezados (puede estar en las primeras 5 filas)
      ...
      for (let rowNum = 1; rowNum <= Math.min(5, worksheet.rowCount); rowNum++) {
        ...
        testRow.eachCell((cell, colNumber) => {
          ...
          testHeaders[colNumber] = value;
        });

        // Mapeo específico para el formato de Excel de Omar
        // Columnas exactas: Fecha, Folio2, CLIENTE, PRODUCTO, CANTIDAD, PRECIO UNITARIO, IMPORTE, AÑOS, MES, VENTA 2024, VENTA 2025
        const possibleColumns = {
          // Cliente - nombres exactos y variaciones
          'cliente': 'client_name',
          'client': 'client_name',
          // Producto - nombres exactos y variaciones
          'producto': 'product_name',
          'product': 'product_name',
          // Cantidad - nombres exactos y variaciones
          'cantidad': 'quantity',
          'qty': 'quantity',
          'volumen': 'quantity',
          'venta 2024': 'quantity_2024',
          'venta 2025': 'quantity_2025',
          // Fecha - nombres exactos y variaciones
          'fecha': 'sale_date',
          'date': 'sale_date',
          'fecha venta': 'sale_date',
          // Folio/Factura - nombres exactos y variaciones
          'folio2': 'invoice_number',
          'folio': 'invoice_number',
          'factura': 'invoice_number',
          'invoice': 'invoice_number',
          'no factura': 'invoice_number',
          // Año y Mes - ya calculados en el Excel
          'años': 'sale_year',
          'año': 'sale_year',
          'year': 'sale_year',
          'mes': 'sale_month',
          'month': 'sale_month',
          // Precio e Importe (opcionales, para referencia)
        ...
        Object.keys(testHeaders).forEach((colNumStr) => {
          ...
          for (const [excelCol, dbField] of Object.entries(possibleColumns)) {
            if (normalizedHeader.includes(excelCol) || normalizedHeader === excelCol) {
              testMap[dbField] = colNum;
              break;
            }
          }
        });

        // Si encontramos al menos cliente y producto, usamos esta fila como encabezado
        if (testMap.client_name && testMap.product_name) {
          headerRowIndex = rowNum;
          headers = testHeaders;
          columnMap = testMap;
          console.log(`✅ [Sales Upload] Encabezados encontrados en fila ${rowNum}:`, Object.keys(columnMap).join(', '));
          console.log(`📋 [Sales Upload] Mapeo completo:`, JSON.stringify(columnMap, null, 2));
          break;
        }
      }

      if (!columnMap.client_name || !columnMap.product_name) {
        return res.status(400).json({
          error: 'Formato de Excel no reconocido',
          details: 'No se encontraron las columnas requeridas (CLIENTE, PRODUCTO). Por favor verifica que el archivo tenga los encabezados correctos.'
        });
      }
      ...
      // Función auxiliar para procesar números del Excel
      ...
      for (let rowNum = headerRowIndex + 1; rowNum <= worksheet.rowCount; rowNum++) {
        ...
        const clientName = row.getCell(columnMap.client_name)?.value?.toString().trim();
        const productName = row.getCell(columnMap.product_name)?.value?.toString().trim();
        
        // Priorizar VENTA 2025, luego VENTA 2024, luego CANTIDAD
        let quantityValue = null;
        let quantity2024 = null;
        let quantity2025 = null;
        
        if (columnMap.quantity_2025) {
          quantity2025 = row.getCell(columnMap.quantity_2025)?.value;
          quantityValue = quantity2025;
        }
        if (columnMap.quantity_2024) {
          quantity2024 = row.getCell(columnMap.quantity_2024)?.value;
          if (!quantityValue) {
            quantityValue = quantity2024;
          }
        }
        if (!quantityValue && columnMap.quantity) {
          quantityValue = row.getCell(columnMap.quantity)?.value;
        }
        
        const saleDateValue = row.getCell(columnMap.sale_date)?.value;
        const invoiceNumber = columnMap.invoice_number 
          ? row.getCell(columnMap.invoice_number)?.value?.toString().trim() 
          : null;
        const folio = columnMap.invoice_number 
          ? row.getCell(columnMap.invoice_number)?.value?.toString().trim() 
          : null;
        const unit = columnMap.unit 
          ? row.getCell(columnMap.unit)?.value?.toString().trim() 
          : (resolvedCompanyId === 1 ? 'KG' : 'unidades');
        const notes = columnMap.notes 
          ? row.getCell(columnMap.notes)?.value?.toString().trim() 
          : null;
        
        // Obtener PRECIO UNITARIO e IMPORTE
        let unitPrice = null;
        let totalAmount = null;
        if (columnMap.unit_price) {
          unitPrice = parseNumber(row.getCell(columnMap.unit_price)?.value);
        }
        if (columnMap.amount) {
          totalAmount = parseNumber(row.getCell(columnMap.amount)?.value);
        }
        
        // Obtener año y mes si están en el Excel (columnas AÑOS y MES)
        let saleYear: number | null = null;
        let saleMonth: number | null = null;
        if (columnMap.sale_year) {
          const yearValue = row.getCell(columnMap.sale_year)?.value;
          if (typeof yearValue === 'number') {
            saleYear = yearValue;
          } else if (typeof yearValue === 'string') {
            saleYear = parseInt(yearValue.trim()) || null;
          }
        }
        if (columnMap.sale_month) {
          const monthValue = row.getCell(columnMap.sale_month)?.value;
          if (typeof monthValue === 'number') {
            saleMonth = monthValue;
          } else if (typeof monthValue === 'string') {
            saleMonth = parseInt(monthValue.trim()) || null;
          }
        }
        ...
        // Procesar cantidad principal
        let quantity = parseNumber(quantityValue) || 0;
        
        // Procesar quantity_2024 y quantity_2025
        const qty2024 = parseNumber(quantity2024);
        const qty2025 = parseNumber(quantity2025);
        ...
        // Procesar fecha - formato esperado: DD/MM/YY o DD/MM/YYYY
        let saleDate: Date;
        if (saleDateValue instanceof Date) {
          ...
          saleDate = new Date(excelEpoch.getTime() + saleDateValue * 86400000);
        } else if (typeof saleDateValue === 'string') {
          // Intentar parsear formato DD/MM/YY o DD/MM/YYYY
          const dateStr = saleDateValue.trim();
        ...
        // Usar año y mes del Excel si están disponibles, sino calcular desde la fecha
        const finalYear = saleYear || saleDate.getFullYear();
        const finalMonth = saleMonth || (saleDate.getMonth() + 1);
        
        // Calcular número de semana (ISO 8601)
        const d = new Date(Date.UTC(finalYear, finalMonth - 1, saleDate.getDate()));
        const dayNum = d.getUTCDay() || 7;
        d.setUTCDate(d.getUTCDate() + 4 - dayNum);
        const yearStart = new Date(Date.UTC(d.getUTCFullYear(), 0, 1));
        const saleWeek = Math.ceil((((d.getTime() - yearStart.getTime()) / 86400000) + 1) / 7);

        // Buscar client_id si existe
        const clientResult = await sql(`
          SELECT id FROM clients 
          WHERE company_id = $1 AND LOWER(name) = LOWER($2)
          LIMIT 1
        `, [resolvedCompanyId, clientName]);
        ...
        if (productResult[0]) {
          ...
          const newProduct = await sql(`
            ...
            RETURNING id
          `, [resolvedCompanyId, productName, unit]);
        ...
        salesRecords.push({
          ...
          quantity_2024: qty2024,
          quantity_2025: qty2025,
          notes,
          upload_id: uploadId
        });
      ...
      console.log(`📊 [Sales Upload] Procesadas ${processedRows} filas válidas, ${skippedRows} filas omitidas`);

      if (salesRecords.length === 0) {
        // Actualizar status del upload a error
        await sql(`
          UPDATE sales_uploads 
          SET status = 'error', notes = 'No se encontraron registros válidos en el archivo'
          WHERE id = $1
        `, [uploadId]);

        return res.status(400).json({
          error: 'No se encontraron datos válidos',
          details: 'El archivo Excel no contiene registros de ventas válidos'
        });
      }

      // Insertar registros en batch
      console.log(`💾 [Sales Upload] Insertando ${salesRecords.length} registros en la base de datos...`);
      
      for (const record of salesRecords) {
        await sql(`
          INSERT INTO sales_data (
            company_id, client_id, client_name, product_id, product_name,
            quantity, unit, sale_date, sale_month, sale_year, sale_week,
            invoice_number, folio, unit_price, total_amount,
            quantity_2024, quantity_2025, notes, upload_id
        ...
        `, [
          ...
          record.unit_price,
          record.total_amount,
          record.quantity_2024,
          record.quantity_2025,
          record.notes,
          record.upload_id
        ]);
      }
      ...
      await sql(`
        ...
        WHERE id = $2
      `, [salesRecords.length, uploadId]);

      console.log(`✅ [Sales Upload] Upload completado exitosamente: ${salesRecords.length} registros insertados`);
