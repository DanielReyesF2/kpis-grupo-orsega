...
/** Límites alineados con EcoNova Gateway (packages/api chat) */
const MAX_USER_DISPLAY_NAME = 200;
const MAX_USER_ROLE = 64;
const MAX_USER_CONTEXT = 12_000;
...
function getApiKey(): string {
  return (
    process.env.NOVA_AI_API_KEY ||
    process.env.NOVA_API_KEY ||
    process.env.VITE_NOVA_AI_API_KEY ||
    ""
  );
}
...
export interface NovaAIChatContext {
  ...
  userId?: string;
  companyId?: number;
  /** @deprecated Preferir userContext — se fusiona con userContext hacia EcoNova como user_context */
  ...
  tenantId?: string;
  /** Identidad humana hacia Brain (<tenant_user>) — inyectar desde BFF con sesión JWT */
  userDisplayName?: string;
  /** Rol para permisos/prompt (ej. director, admin, user) */
  userRole?: string;
  /** Contexto libre corto (empresa, área); opcional */
  userContext?: string;
}
...
/** Campos snake_case que el Gateway EcoNova reenvía al Brain */
function appendIdentityToPayload(
  payload: Record<string, unknown>,
  context: NovaAIChatContext,
): void {
  const name = context.userDisplayName?.trim();
  if (name) {
    payload.user_display_name = name.slice(0, MAX_USER_DISPLAY_NAME);
  }
  const role = context.userRole?.trim();
  if (role) {
    payload.user_role = role.slice(0, MAX_USER_ROLE);
  }
  const ctxRaw = (context.userContext ?? context.additionalContext)?.trim();
  if (ctxRaw) {
    payload.user_context = ctxRaw.slice(0, MAX_USER_CONTEXT);
  }
}
...
async function streamChat(
  ...
  const tenantId =
    (context.tenantId && context.tenantId.trim()) || getTenantId();
  ...
  const payload: Record<string, unknown> = {
    ...
    company_id:
      context.companyId !== undefined ? String(context.companyId) : undefined,
  };
  appendIdentityToPayload(payload, context);
  ...
  try {
    ...
    console.log(
      `[NovaClient] Response status: ${response.status}, content-type: ${response.headers.get("content-type")}`,
    );
    ...
    while (true) {
      ...
      if (chunkCount <= 3 || chunkCount % 50 === 0) {
        console.log(
          `[NovaClient] Chunk #${chunkCount} (${chunk.length} bytes): ${chunk.substring(0, 300)}`,
        );
      }
        ...
        if (cleanLine.startsWith("event: ")) {
          ...
          try {
            ...
            switch (eventType) {
              ...
              case "tool_result":
                console.log(
                  `[NovaClient] Tool result: ${data.tool}, success: ${data.success}`,
                );
              ...
              case "done":
                ...
                console.log(
                  `[NovaClient] Done event — answer length: ${(data.answer || "").length}, toolsUsed: ${JSON.stringify(data.toolsUsed || data.tools_used || [])}, conversationId: ${conversationId ?? "—"}`,
                );
                callbacks.onDone({
                  answer: data.answer || "",
              ...
              default:
                ...
                if (eventType) {
                  console.warn(
                    `[NovaClient] Unrecognized event type: "${eventType}", data: ${dataStr.substring(0, 200)}`,
                  );
                }
            }
            ...
            console.warn(
              `[NovaClient] JSON parse failed for line: ${line.substring(0, 200)}`,
            );
          }
          ...
          if (chunkCount <= 5) {
            console.warn(
              `[NovaClient] Non-SSE line: ${line.substring(0, 300)}`,
            );
          }
        }
      }
    ...
    console.warn(
      `[NovaClient] Stream ended without done event. Chunks: ${chunkCount}, tokens: ${tokenCount}, remaining buffer: ${buffer.substring(0, 300)}`,
    );
    callbacks.onDone({
      answer: "",
      toolsUsed: [],
    ...
    const err =
      error instanceof Error ? error : new Error("Error conectando a Nova AI");
...
async function chat(
  ...
  appendIdentityToPayload(bodyPayload, context);
